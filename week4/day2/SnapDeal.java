...
public class SnapDeal {
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		System.out.println("The displayed prices are: ");
		System.out.println(price);//values in this list is String
		
		//we can check the prices sorted correctly are not using this for loop
		/*List<Integer> priceIn=new ArrayList<Integer>();
		 * for(String s : price) {
			priceIn.add(Integer.valueOf(s));
			//this method is used to convert string list into integer list
		}
		System.out.println(priceIn);  
		 
		for(int i=0;i<list.size();i++) {
			for(int j=i+1;j<list.size();j++) {
				if(priceIn.get(i)<priceIn.get(j)) {
